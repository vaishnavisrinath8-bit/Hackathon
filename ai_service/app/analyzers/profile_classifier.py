...
def classify_profile(occupation: str, details: dict) -> dict:
    config = OCCUPATION_ANALYZERS.get(occupation)

    if not config:
        return {
            "track": "Unknown",
            "dashboard_signal": "No analyzer found for this occupation."
        }

    _, analyzer_func = config
    return analyzer_func(details)